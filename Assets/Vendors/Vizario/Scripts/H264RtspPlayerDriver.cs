...
public class H264RtspPlayerDriver : MonoBehaviour
{
    ...
    private bool m_buffering = false; // only works if on RTSP (not LOCAL)!
    ...
    public async void StartPlay()
    {
        ...
        // general RTCM data 
        m_rtspclient.Received_RTCMTime += (bool isVideoChannel, UInt32 rtptimestamp, double ntpasdouble, DateTime ntpasdatetime) =>
        {
            // Debug.Log("RTCM: " + isVideoChannel + " - rtptime: " + rtptimestamp + " - ntpasdouble: " + ntpasdouble);
            m_bridge.Received_RTCMTime(isVideoChannel, rtptimestamp, ntpasdouble, ntpasdatetime);
        };
